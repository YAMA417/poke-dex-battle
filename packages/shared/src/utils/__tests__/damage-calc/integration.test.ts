...
describe("ダメージ計算エンジン統合テスト", () => {
  ...
  function compareResults(
    legacyInput: DamageCalculationInput,
    testName: string,
  ): void {
    // 旧API実行
    ...
    const { attacker, defender, move, context } =
      convertLegacyInput(legacyInput);
    ...
    console.log(`\n${testName}`);
    console.log(
      `旧API - min: ${legacyResult.minDamage}, max: ${legacyResult.maxDamage}`,
    );
    console.log(
      `新API - min: ${v2Result.minDamage}, max: ${v2Result.maxDamage}`,
    );
