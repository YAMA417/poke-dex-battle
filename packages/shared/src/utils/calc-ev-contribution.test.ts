...
describe('calcEvContributionToActualStats', () => {
  ...
  describe('単一ステータスのテスト', () => {
    ...
    it('HP能力P=10 の場合、合計は10', () => {
      expect(calcEvContributionToActualStats(createStats(10, 0, 0, 0, 0, 0))).toBe(10);
    });

    it('攻撃能力P=32 の場合、合計は32', () => {
      expect(calcEvContributionToActualStats(createStats(0, 32, 0, 0, 0, 0))).toBe(32);
    });
  });

  describe('複数ステータスのテスト', () => {
    it('各ステータス11ずつ（合計66）', () => {
      expect(calcEvContributionToActualStats(createStats(11, 11, 11, 11, 11, 11))).toBe(66);
    });

    it('HP=32, 攻撃=32, 残り=2（合計66）', () => {
      expect(calcEvContributionToActualStats(createStats(32, 32, 2, 0, 0, 0))).toBe(66);
    });
  ...
  describe('上限テスト', () => {
    it('全ステータス32の場合、192（理論値、実際には合計66制限）', () => {
      expect(calcEvContributionToActualStats(createStats(32, 32, 32, 32, 32, 32))).toBe(192);
    });
