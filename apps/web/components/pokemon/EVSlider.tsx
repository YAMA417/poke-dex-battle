        ...
        <div className="space-y-3">
            ...
            <div className="space-y-1">
                <div className="flex justify-between text-xs text-gray-600">
                    ...
                    <span className={`font-bold tabular-nums ${remaining === 0 ? 'text-red-500' : 'text-gray-700'}`}>
                        {total} <span className="text-gray-400 font-normal">/ {MAX_EV_TOTAL}</span>
                        {remaining > 0 && (
                            <span className="text-green-600 ml-1 font-normal">（残り{remaining}）</span>
                        )}
                    </span>
                </div>
                <div className="w-full bg-gray-100 rounded-full h-2.5 overflow-hidden">
                    <div
                        className={`h-full rounded-full transition-all duration-200 ${remaining === 0 ? 'bg-red-400' : 'bg-pokemon-blue'
                            }`}
                        style={{ width: `${pct}%` }}
                    />
                </div>
            ...
            <div className="grid grid-cols-3 gap-2">
                {STAT_LABELS.map(({ key, label }) => {
                    ...
                    const labelColorClass = isUp ? 'text-red-500' : isDown ? 'text-blue-500' : 'text-gray-500';

                    return (
                        <div key={key} className="flex flex-col gap-0.5">
                            <label className={`text-[10px] font-medium ${labelColorClass}`}>
                                {displayLabel}
                            </label>
                            <input
                                type="number"
